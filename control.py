...
async def downloadFiles(
    client, info, password, originalPath, req, layers, start=1, num=-1, _id=0
):
    ...
    if password != "":
        ...
        pheader, temp_url = asyncio.get_event_loop().run_until_complete(
            getpassword(originalPath, password)
        )
    ...
    if filesData == None:
        pat = re.search(
            'g_listData = {"wpq":"","Templates":{},"ListData":{ "Row" : ([\s\S]*?),"FirstRow"',
            reqf.text,
        )
    ...
    for i in filesData:
        if i["FSObjType"] == "1":
            ...
            if num == -1 or start <= fileCount + _id < start + num:
                ...
                download_path = (
                    f"/root/Download{str(query['id']).split('Documents', 1)[1]}"
                )
...
async def odshare_download(client, message):
    ...
    try:
        ...
        info = await client.send_message(
            chat_id=message.chat.id, text="开始抓取下载链接", parse_mode="markdown"
        )
        fileCount = await downloadFiles(
            client, info, password, odshare_url, None, 0, start=1, num=-1
        )
        await client.edit_message_text(
            text=f"推送至Aria2完成，可到AriaNG面板查看\n本次推送{fileCount}个任务",
            chat_id=info.chat.id,
            message_id=info.message_id,
            parse_mode="markdown",
        )
        ...
        await client.send_message(
            chat_id=message.chat.id, text="抓取下载链接失败", parse_mode="markdown"
        )
...
async def login_of_share(client, info, link, admin, password):
    try:
        ...
        await client.send_message(
            chat_id=info.chat.id, text=f"login_of_share {e}", parse_mode="markdown"
        )


async def odpriva_downloadFiles(
    client, info, admin, password, originalPath, req, layers, start=1, num=-1, _id=0
):
    try:
        header = {}
        if req == None:
            req = requests.session()
            header, originalPath = await login_of_share(
                client, info, originalPath, admin=admin, password=password
            )
            if originalPath == "":
                await client.edit_message_text(
                    text=f"登录错误",
        ...
        if (
            "NextHref"
            in graphqlReq["data"]["legacy"]["renderListDataAsStream"]["ListData"]
        ):
            nextHref = graphqlReq["data"]["legacy"]["renderListDataAsStream"][
                "ListData"
            ]["NextHref"] + "&@a1=%s&TryNewExperienceSingle=TRUE" % (
                "%27" + relativeUrl + "%27"
            )
            filesData.extend(
                graphqlReq["data"]["legacy"]["renderListDataAsStream"]["ListData"][
                    "Row"
                ]
            )
            # print(filesData)
            ...
            while "NextHref" in graphqlReq["ListData"]:
                nextHref = graphqlReq["ListData"][
                    "NextHref"
                ] + "&@a1=%s&TryNewExperienceSingle=TRUE" % (
                    "%27" + relativeUrl + "%27"
                )
                filesData.extend(graphqlReq["ListData"]["Row"])
                graphqlReq = req.post(
        ...
        for i in filesData:
            if i["FSObjType"] == "1":
                ...
                fileCount += await odpriva_downloadFiles(
                    client,
                    info,
                    admin,
                    password,
        ...
        await client.send_message(
            chat_id=info.chat.id,
            text=f"odpriva_downloadFiles {e}",
            parse_mode="markdown",
        )
...
async def odprivate_download(client, message):
    try:
        ...
        fileCount = await odpriva_downloadFiles(
            client, info, admin, password, odprivate_url, None, 0, start=1, num=-1
        )
        await client.edit_message_text(
            text=f"推送至Aria2完成，可到AriaNG面板查看\n本次推送{fileCount}个任务",
            chat_id=info.chat.id,
        ...
        await client.send_message(
            chat_id=message.chat.id, text=f"odprivate error {e}", parse_mode="markdown"
        )
...
async def run_await_rclone(dir, title, info, file_num, client, message, gid):
    ...
    info = await client.send_message(
        chat_id=message.chat.id, text="开始上传", parse_mode="markdown"
    )
    ...
    if int(file_num) == 1:
        ...
        while (
            requests.post(url=rcd_status_url, json={"jobid": jobid}).json()["finished"]
            == False
        ):

            job_status = requests.post(
                url=f"{rc_url}/core/stats", json={"group": f"job/{jobid}"}
            ).json()
            print(job_status)
    ...
    requests.post(
        url=f"{rc_url}/core/stats-delete", json={"group": f"job/{jobid}"}
    ).json()
...
def the_download(client, message, url):
    ...
    try:
        ...
        if str(e).endswith("No URI to download."):
            ...
            client.send_message(
                chat_id=message.chat.id, text="No link provided!", parse_mode="Markdown"
            )
            return None
    prevmessagemag = None
    info = client.send_message(
        chat_id=message.chat.id, text="添加任务", parse_mode="markdown"
    )

    inline_keyboard = [
        [InlineKeyboardButton(text=f"Remove", callback_data=f"Remove {download.gid}")]
    ]
            ...
            try:
                ...
                if str(e).endswith("is not found"):
                    ...
                    if temp_text != "Metadata Cancelled/Failed":
                        try:
                            client.edit_message_text(
                                text="Metadata Cancelled/Failed",
                                chat_id=info.chat.id,
                                message_id=info.message_id,
                                parse_mode="Markdown",
                            )
    ...
    new_inline_keyboard = [
        [
            InlineKeyboardButton(
                text="Resume", callback_data=f"Resume {currdownload.gid}"
            ),
            InlineKeyboardButton(
                text=f"Pause", callback_data=f"Pause {currdownload.gid}"
            ),
            InlineKeyboardButton(
                text=f"Remove", callback_data=f"Remove {currdownload.gid}"
            ),
        ]
    ]
    ...
    while True:
        ...
        try:
            ...
            if str(e).endswith("is not found"):
                ...
                try:
                    client.edit_message_text(
                        text="Magnet download was removed",
                        chat_id=info.chat.id,
                        message_id=info.message_id,
                        parse_mode="markdown",
                    )
        ...
        if currdownload.status == "removed":
            ...
            try:
                client.edit_message_text(
                    text="Magnet download was cancelled",
                    chat_id=info.chat.id,
                    message_id=info.message_id,
                    parse_mode="markdown",
                )
...
def start_http_download(client, message):
    try:
        ...
        if str(BOT_name) in keywords:
            ...
            print(keywords)
            t1 = threading.Thread(
                target=http_download, args=(client, message, keywords)
            )
            t1.start()
        else:
...
def file_download(client, message, file_dir):
    ...
    try:
        ...
        info = client.send_message(
            chat_id=message.chat.id, text="开始下载", parse_mode="markdown"
        )
        print("发送信息")
        sys.stdout.flush()
    except Exception as e:
...
async def more_magnet(client, message):
    try:
        ...
        if "magnet" in text:
            if "\n" in text:
                ...
                for magnet in magnet_list:
                    t1 = threading.Thread(
                        target=the_download, args=(client, message, magnet)
                    )
                    t1.start()
        ...
        await client.send_message(
            chat_id=message.chat.id, text=f"more_magnet error :{e}"
        )
    ...
    try:
        if len(file_list) == 0:
            answer = await client.ask(
                chat_id=message.chat.id, text="请发送TG文件,或输入 /cancel 取消"
            )
        else:
            answer = await client.ask(
                chat_id=message.chat.id,
        ...
        if info.media_group_id != None:
            media = await client.get_media_group(
                chat_id=info.chat.id, message_id=info.message_id
            )
            print(media)
            for a in media:
                if a.document == None and a.video == None:
                    await client.send_message(
                        text="发送的不是文件", chat_id=message.chat.id, parse_mode="markdown"
                    )
            ...
            await client.send_message(
                text="取消发送", chat_id=message.chat.id, parse_mode="markdown"
            )
            return []
        elif info.text == "/finish":
            await client.send_message(
                text=f"接收文件完成,共有{len(file_list)}个文件",
                chat_id=message.chat.id,
                parse_mode="markdown",
            )
            return file_list
            ...
            try:
                ...
                sys.stdout.flush()
                await client.send_message(
                    text="下载文件失败", chat_id=message.chat.id, parse_mode="markdown"
                )
                return file_list
    except Exception as e:
        print(f"下载文件失败 {e}")
...
async def send_telegram_file(client, message):
    try:
        ...
        if len(temp) == 0:
            ...
            t1 = threading.Thread(
                target=file_download, args=(client, message, file_dir)
            )
            t1.start()
            return
        ...
        await client.send_message(
            text=f"下载文件失败:{e}", chat_id=message.chat.id, parse_mode="markdown"
        )
...
def start_http_downloadtg(client, message):
    try:
        ...
        if str(BOT_name) in keywords:
            ...
            print(keywords)
            t1 = threading.Thread(
                target=http_downloadtg, args=(client, message, keywords)
            )
            t1.start()
        else:
