...
def change_password():
    ...
    zh_data = {
        ...
        "what": "user",
        "which": ["locale", "hideDotfiles", "singleClick"],
    }
    ...
    change_data = {
        "data": {"id": 1, "password": str(Aria2_secret)},
        "what": "user",
        "which": ["password"],
    }
...
client = Client("my_bot", bot_token=Telegram_bot_api, api_hash=Api_hash, api_id=Api_id)
...
if App_title != "" and "-" not in str(Telegram_user_id):
    ...
    client.send_message(
        chat_id=int(Telegram_user_id), text=f"Bot上线！！！\nAria2NG快捷面板：{ari2_ng_url}"
    )
