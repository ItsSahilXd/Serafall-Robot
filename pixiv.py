...
async def start_download_pixiv_top(client, call):
    ...
    info = await client.send_message(
        chat_id=message_chat_id, text="开始下载", parse_mode="markdown"
    )
    print(info)
    ...
    if "tg" in str(keywords):
        ...
        await client.send_message(
            chat_id=message_chat_id, text="开始发送，请等待", parse_mode="markdown"
        )
        try:
            img_list = []
            for root, dirs, files in os.walk(author):
                for file in files:
                    try:
                        ...
                        if (
                            float(float(img.size[0]) / float(img.size[1])) > 20
                            or float(img.size[0]) + float(img.size[1]) > 10000
                        ):
                            os.remove(file_dir)
                            continue
                        ...
                        if os.path.getsize(file_dir) < 1024 * 1024 * 10:
                            img_list.append(
                                InputMediaPhoto(media=file_dir, caption=file)
                            )
                        else:
                        ...
                        if len(img_list) == 10:
                            await client.send_chat_action(
                                chat_id=message_chat_id, action="upload_photo"
                            )
                            print("开始上传")
                            sys.stdout.flush()
                            await client.send_media_group(
                                chat_id=message_chat_id, media=img_list
                            )
                            img_list = []
                    except Exception as e:
                        print(f"标记3 {e}")
        ...
        del_path(author)
        await client.send_message(
            chat_id=message_chat_id, text="本次发送完成", parse_mode="markdown"
        )
        return
        ...
        try:
            ...
            for a, b in zip(name_list, img_list):
                put_text = (
                    put_text + f'<strong>{a}</strong><br /><img src="{b}" /><br>\n\n'
                )
            print(put_text)
...
async def pixiv_topillustration(client, message):
    try:
        ...
        text = (
            f"当前下载插画排行榜，请选择下载选项，当前下载页数为{page_num}页\n一页包含50张图片"
            f"1-打包到网盘(插画日榜)\n"
            f"2-打包到网盘(插画周榜)\n"
            f"3-打包到网盘(插画月榜)\n"
            f"4-打包发送给我(插画日榜)\n"
...
async def pixiv_topall(client, message):
    try:
        ...
        new_inline_keyboard = [
            [
                InlineKeyboardButton(
                    text="1", callback_data=f"pixivtopdayall {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text="2", callback_data=f"pixivtopweekall {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text="3", callback_data=f"pixivtopmonthall {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"4", callback_data=f"pixivtopdayallltg {page_num} {rand_data}"
                ),
            ],
            [
                InlineKeyboardButton(
                    text=f"5", callback_data=f"pixivtopweekalltg {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"6",
            ...
            [
                InlineKeyboardButton(
                    text=f"13", callback_data=f"pixivtopmanmale {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"14",
            ...
            [
                InlineKeyboardButton(
                    text=f"17", callback_data=f"pixivtopfemale {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"18", callback_data=f"pixivtopfemaletg {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"19",
            ...
            [
                InlineKeyboardButton(
                    text=f"21", callback_data=f"pixivtoprookie {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"22", callback_data=f"pixivtoprookietg {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"23",
            ...
            [
                InlineKeyboardButton(
                    text=f"25", callback_data=f"pixivtoporiginal {page_num} {rand_data}"
                ),
                InlineKeyboardButton(
                    text=f"26",
        ...
        text = (
            f"当前下载综合排行榜，请选择下载选项，当前下载页数为{page_num}页\n一页包含50张图片\n"
            f"1-打包到网盘(日榜综合)\n"
            f"2-打包到网盘(周榜综合)\n"
            f"3-打包到网盘(月榜综合)\n"
            f"4-打包发送给我(日榜综合)\n"
...
async def author(client, message):
    try:
        ...
        author_text = (
            f"画师:`{author_json['user'][str(author_id)]['name']}`\n"
            f"画师ID:`{author_id}`\n"
        ...
        new_inline_keyboard = [
            [
                InlineKeyboardButton(
                    text="打包到网盘", callback_data=f"pixivuser {author_id}"
                ),
                InlineKeyboardButton(
                    text=f"打包发送给我", callback_data=f"pixivusertg {author_id}"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="发送图片给我", callback_data=f"pixivuserphoto {author_id}"
                ),
                InlineKeyboardButton(
                    text=f"网页方式发送", callback_data=f"pixivusertele {author_id}"
                ),
            ],
        ]
